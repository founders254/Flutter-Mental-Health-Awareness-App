...
class YouTubeService {
  ...
  // Fetch sounds for a specific category
  Future<List<Map<String, String>>> _fetchSoundsByCategory(String query) async {
    final url = Uri.parse(
        '$baseUrl?part=snippet&q=$query&type=video&maxResults=20&key=$apiKey');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load sounds for $query');
      }
    }
  }

  // Public method to fetch sounds for all categories
  Future<Map<String, List<Map<String, String>>>> fetchAllSounds() async {
    final categories = ['nature sounds', 'rain sounds', 'thunderstorm sounds', 'ocean waves', 'tin roof sounds'];

    Map<String, List<Map<String, String>>> results = {};
    for (final category in categories) {
      results[category] = await _fetchSoundsByCategory(category);
    }
    return results;
  }
